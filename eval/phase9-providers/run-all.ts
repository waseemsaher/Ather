// ─────────────────────────────────────────────────────────────
// AETHER Eval — Phase 9: Multi-Provider Support Runner
// ─────────────────────────────────────────────────────────────

import { TestHarness, writeReport, type PhaseReport } from "../helpers/test-harness.ts";

const TESTS = [
  { id: "01", mod: () => import("./01-provider-construction.test.ts") },
  { id: "02", mod: () => import("./02-provider-manager.test.ts") },
  { id: "03", mod: () => import("./03-model-aliases.test.ts") },
];

export async function run(): Promise<PhaseReport> {
  const harness = new TestHarness("Phase 9: Multi-Provider Support");
  harness.start();
  for (const test of TESTS) {
    try {
      ...
      await harness.runTest(`9.${test.id}.0`, `${test.id} load`, async () => ({
